return {
...

    {
      ...
      -- Plenary is required, but you probably already have it if you use Telescope
      dependencies = {
        "nvim-lua/plenary.nvim",
      },
      -- This sets the keymap and tells Lazy to only load the plugin when you press the keys
      keys = {
...

    {
      ...
      init = function()
        -- Hooking it up to that fresh Nerd Font you just installed
        vim.g.db_ui_use_nerd_fonts = 1
        vim.g.db_ui_show_database_icon = 1

        -- Tells Dadbod to save your DB connections locally so they don't get pushed to GitHub
        vim.g.db_ui_save_location = vim.fn.stdpath("config") .. "/db_ui"
      end,
      keys = {
        -- Maps the toggle to Space + db
